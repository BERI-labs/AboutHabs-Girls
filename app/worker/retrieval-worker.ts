...
//       If Habs Girls uses a different term (e.g. "financial aid"), add synonyms here.
...
let docTermFreqs: { terms: Map<string, number>; length: number }[] = [];
let avgDocLength = 0;
...
// ── Markdown parser / chunker (fallback when habs-girls-chunks.json unavailable)
...
async function _init() {
  // 1. Load pre-built habs-girls-chunks.json (primary path)
  try {
    const response = await fetch(`${basePath}/data/habs-girls-chunks.json`);
    if (response.ok) {
  ...
  if (chunks.length === 0) {
    try {
      const response = await fetch(
        `${basePath}/data/HabsGirls_Merged_Dataset_Final.md`,
      );
